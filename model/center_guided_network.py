...
class HighResolutionModule(nn.Module):
    def __init__(self, num_branches, blocks, num_blocks, num_inchannels,
        ...
        # self.fuse_weight = nn.Parameter(torch.ones(num_branches, num_branches))
        self.relu = nn.ReLU(inplace=True)
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.center_guide_channel_module_3 = _CenterGuidedChannelModule(channel=pre_stage_channels[3])
    ...
    def forward(self, x):
        ...
        x3 = self.center_guide_channel_module_3(x3, skl_feature)
        x0 = x0 * (1 + skl_feature)
        x1 = x1 * (1 + skl_feature)
        x2 = x2 * (1 + skl_feature)
        x3 = x3 * (1 + skl_feature)
        x_seg = torch.cat((x0, x1, x2, x3), dim=1)
        x_seg = self.last_layer(x_seg)
        x_seg = F.interpolate(x_seg, size=(512, 512), mode='bilinear', align_corners=False)
        return x_seg, skl_feature


class _CenterGuidedChannelModule(nn.Module):
    def __init__(self, channel):
        super(_CenterGuidedChannelModule, self).__init__()
        self.recalibration = nn.Sequential(nn.Linear(channel, channel//2, bias=True),
                                           nn.ReLU(inplace=True),
                                           nn.Linear(channel//2, channel, bias=True),
                                           nn.Softmax(),
                                          )

    def forward(self, x, skl_feature):
        batch_size, channel, height, width = x.size()
        f_mask_pooling = torch.sum(torch.sum(x * skl_feature, dim=3), dim=2) / \
                         torch.sum(torch.sum(skl_feature, dim=3), dim=2)
        weight = self.recalibration(f_mask_pooling).view(batch_size, channel, 1, 1)
...
class _CenterInformationExtractionModule(nn.Module):
    def __init__(self, channel):
        ...
        self.conv_stage1_2 = nn.Sequential(nn.Conv2d(channel, 2 * channel, 3, padding=1, bias=True),
                                           gn(2 * channel))
        self.conv_stage1_3 = nn.Sequential(nn.Conv2d(channel, 2 * channel, 3, padding=1, bias=True),
                                           gn(2 * channel))
        self.conv_stage2_1 = nn.Sequential(nn.Conv2d(2 * channel, 4 * channel, 3, padding=1, bias=True),
                                           gn(4 * channel))
        self.conv_stage2_2 = nn.Sequential(nn.Conv2d(2 * channel, 4 * channel, 3, padding=1, bias=True),
                                           gn(4 * channel))
        self.output = nn.Sequential(nn.Conv2d(4 * channel, 4 * channel, kernel_size=1, stride=1, padding=0),
                                    gn(4 * channel),
                                    nn.ReLU(inplace=True),
                                    nn.Conv2d(in_channels=4 * channel,
                                              out_channels=1,  # 类别数
                                              kernel_size=1,
                                              stride=1,
                                              padding=0))
    ...
    def forward(self, x):
        F1_1 = self.conv_stage1_1(x[0])
        F1_2 = self.conv_stage1_2(x[1])
        F1_3 = self.conv_stage1_3(x[2])
        F2_1 = self.conv_stage2_1(self.relu(F1_1 + F1_2))
        F2_2 = self.conv_stage2_2(self.relu(F1_2 + F1_3))
        skl_feature = self.sigmoid(self.output(F2_1 + F2_2))
        return skl_feature
...
def _hrnet(arch, pretrained, progress, **kwargs):
    ...
    if pretrained:
        model_url = model_urls[arch + '_imagenet']
